...
class Queueing: # Инициализация очереди
    ...
    def __get_free_device_number__(self): # Поиск первого свободного устройства
        for i, device in enumerate(self.devices):
            if not device['claim']:
                return i
        return -1

    def __get_queue_length__(self): # Количество заявок в очереди
        length = 0
        for claims in self.storage:
            length += len(claims)
        return length

    def __get_eviction_candidate__(self): # Кандидат на замену
    ...
    def __occupy_device__(self, cur_time, device_number, claim): # Заявка на устройство
        claim.processing_start_time = cur_time
        self.devices[device_number]['claim'] = claim

    def __unclaim_device__(self, cur_time, device_number): # Освобождение устройства
    ...
    def add_claim(self, receiving_time, priority, service_time): # Создание новой заявки
    ...
    def get_usage_rate(self, time): # Расчет коэффициента использования устройств
        return round(sum(map(lambda d: d['busy_time'], self.devices)) / time / len(self.devices), 2)

    def get_received_claims_number(self): # Получение общего количества полученных заявок
        return self.get_stay_claims_number() + len(self.refusals) + len(self.served_claims)

    def get_average_queue_time(self, cur_time): # Расчет среднего времени ожидания заявок в очереди
        queue_claims_time = 0
        for claims in self.storage:
            for claim in claims:
    ...
    def get_queue_claims_number(self): # Получение количества заявок в очереди
        number = 0
        for claims in self.storage:
            number += len(claims)
        return number

    def get_stay_claims_number(self): # Получение количества заявок, находящихся в очереди или на обслуживании
