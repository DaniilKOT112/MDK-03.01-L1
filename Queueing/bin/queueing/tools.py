...
def is_pos_float(s): # Проверка положительности чисел с плавающей точкой
    try:
        float(s)
        return float(s) > 0
    except ValueError:
...
def is_pos_int(s): # Проверка положительности целых чисел
    try:
        int(s)
        return int(s) > 0
    except ValueError:
...
def get_intervals_0_1(p): # Генерация интервалов (гистограмма)
    res = [[0, p[0]]]
    amount = p[0]
...
def get_mx(x): # Среднее значение списка
    return round(sum(x) / len(x), 2)


def get_dx(x): # Дисперсия списка
    mx = get_mx(x)
    return round(sum([(x_i - mx) ** 2 for x_i in x]) / (len(x) - 1), 2)


def get_p_mx(p): # Математическое ожидание (дискретное распределение)
    return round(sum([(x_i + 1) * p_i for x_i, p_i in enumerate(p)]), 2)


def get_p_mx2(p): # Расчет математического ожидания квадрата
    return round(sum([(x_i + 1)**2 * p_i for x_i, p_i in enumerate(p)]), 2)


def get_p_dx(p): # Расчет дисперсии (Дисконтное распределение)
    return round(get_p_mx2(p) - get_p_mx(p)**2, 2)


def covariance(x, j): # Ковариация
    mx = get_mx(x)
    n = len(x)
    return sum([(x[i] - mx) * (x[i + j] - mx) for i in range(1, n - j)]) / (n - j)


def correlation_plot(x): # Построение графика корреляции
    dx = get_dx(x)
    res = []
...
def scatter_plot(x): # Построение диаграммы разброса
...
def confidence_interval(x): #Доверительный интервал
    k = 1.96 * (get_dx(x) / len(x)) ** 0.5
    mx = get_mx(x)
    return round(mx - k, 3), round(mx + k, 3)


def get_histogram_x_points(a, b, dx): # Генерация точек для построения гистограммы
    res = []
    i = a
    while i < b:
...
def get_histogram_n(x, intervals): # Расчет частот для гистограммы
...
def plot_histogram(x, lmbd, dx, intervals, n): # Построение графика плотности вероятности
...
def frequency_plot(x): # Построение графика относительной частоты
    freq = {}
    for x_i in x:
